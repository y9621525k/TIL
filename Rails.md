...
## ルーティングのnamespace
- ControllerやView、routingで名前の重複があるとき、名前付けの衝突を防ぐために使用する。
- 管理者権限でしか閲覧できないページなど、ページ毎に意味のまとまりを持たせたい場面で使用する。（/admin/users など）
- これによりユーザーがアクセスできるページをURLレベルで分けることができる。
...
```
e.g.
ユーザーの一覧ページを表示したい = ユーザーのリストを取得する → GET
ユーザーの登録を行いたい = メールアドレスやパスワードを入力しサーバに送信する → POST
```
- GET時のクエリはブラウザのURLに追記される(?prefecture=tokyo)
- POST時のパラメータはHTTPリクエストメッセージボディに含まれる

## HTTPステータス（404, 503など）
### 200番台・・・正常にレスポンスが返されている場合
### 300番台・・・リダイレクト
- 301・・・恒久的なリダイレクト。サイト移転などの場合に使います。
- 302・・・一時的なリダイレクト。リニューアルやメンテナンス等で一時的に別ページに飛ばしたい場合に使います。SEOの観点で両者の使い分けが重要になってきます。
### 400台・・・クライアント側に問題がある場合
- 400・・・Bad Request。必須項目を送信していないなどリクエスト内容に誤りがある。
- 401・・・Unauthorized。認証が必要なページに認証せずにリクエストを投げた場合など。
- 403・・・Forbidden。アクセス権限が無くリクエストを拒否されたなど。
- 404・・・Not Found。 ページが見つからない。
### 500台・・・サーバー側に問題がある場合。
- 500・・・Internal Server Error。サーバ側のプログラムのバグなどで正常に処理を実行できない場合など。
- 502・・・Bad Gateway。アプリケーションサーバまでの経路で何か問題がある場合など。
- 503・・・Service Unavailable。サーバが落ちてる場合など。
